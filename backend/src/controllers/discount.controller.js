...
async function create(req, res, next) {
  try {
    return success(res, await model.create(req.body), 201);
  } catch (e) {
    next(e);
  }
}
...
async function update(req, res, next) {
  try {
    ...
    return row
      ? success(res, row)
      : failure(res, "Discount rule not found", 404);
  } catch (e) {
    next(e);
  }
}
async function evaluate(req, res, next) {
  try {
    return success(res, await engine.evaluate(req.params.quotationId));
  } catch (e) {
    next(e);
  }
}
